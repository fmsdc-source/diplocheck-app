...
<style>
    ...
    .rank-other { background: #1e3050; color: #6a8ab4; }
    .leader-bar-bg {
        width: 120px; height: 6px;
        background: #1e3050; border-radius: 3px; overflow: hidden;
    }
    .leader-bar-fill {
...
def render_plate_header():
    """Render the DiploCheck plate stripe header."""
    st.markdown("""
    <div class="plate-frame" style="margin-bottom:4px;">
        <div class="plate-inner">
            <div class="plate-stripe">
                <div class="plate-bolt" style="top:4px;left:8px;"></div>
                <div class="plate-bolt" style="top:4px;right:8px;"></div>
...
def display_result(vehicle_type, code, country, is_ambassador, plate_text=""):
    """Show result as a styled plate + flag + map + trivia."""
    if is_ambassador:
        st.balloons()
    ...
    display_plate = plate_text.upper().replace(" ", "") if plate_text else f"{code}"
    ...
    if flag_url:
        ...
        flag_html = f'<span style="font-size:32px;">{icon}</span>'

    type_label = vehicle_type
    if is_ambassador:
        type_label = "👑 " + vehicle_type
    ...
    <div class="plate-frame result-plate" style="margin-top:16px;">
        <div class="plate-inner">
            ...
            <div class="plate-bolt" style="bottom:6px;left:8px;"></div>
            <div class="plate-bolt" style="bottom:6px;right:8px;"></div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
if st.session_state.page == "home":
    ...
    # Plate-styled header as hero
    st.markdown("""
    <div class="plate-frame" style="max-width:420px; margin:0 auto;">
        <div class="plate-inner">
            <div class="plate-stripe">
                <div class="plate-bolt" style="top:4px;left:8px;"></div>
                <div class="plate-bolt" style="top:4px;right:8px;"></div>
            ...
            <div style="padding:24px 20px; text-align:center;">
                <div class="plate-bolt" style="bottom:6px;left:8px;"></div>
                <div class="plate-bolt" style="bottom:6px;right:8px;"></div>
                <p style="font-size:28px; font-weight:700; color:#0a1628 !important; letter-spacing:0.1em; margin:0 0 6px; font-family:monospace;">
                    DIPLOCHECK
                </p>
                <p style="font-size:12px; color:#6b7a8d !important; letter-spacing:0.04em; margin:0; text-transform:uppercase;">
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    # Text input
    st.markdown('<div class="dark-card"><h3>⌨️ Type a plate number</h3><p class="subtitle">Letters and numbers — we\'ll figure out the format</p></div>', unsafe_allow_html=True)
    ...
    if manual_input:
        ...
        if code and country != "Unknown Code":
            ...
            display_result(vehicle_type, code, country, is_ambassador, manual_input)
        else:
    ...
    # Upload / camera
    st.markdown('<div class="dark-card"><h3>📷 Upload or take a photo</h3><p class="subtitle">Snap a plate and let AI read it for you</p></div>', unsafe_allow_html=True)
        ...
        with st.spinner("🔍 Analyzing with Google AI..."):
            ...
            try:
                ...
                if ai_result == "NONE":
                    ...
                    if code and country != "Unknown Code":
                        record_scan(code, country)
                        display_result(vehicle_type, code, country, is_ambassador, ai_result)
            ...
            if len(df) == 0:
                ...
                for i, row in leaderboard.iterrows():
                    ...
                    <div class="leader-row">
                        ...
                        {flag_html}
                        <span style="font-size:14px; font-weight:600; color:#c0cde0 !important; flex:1;">{row['Country']}</span>
                        <div class="leader-bar-bg">
                            <div class="leader-bar-fill" style="width:{pct}%;"></div>
                        </div>
                        <span style="font-size:14px; font-weight:700; color:#ffffff !important; min-width:28px; text-align:right;">{int(row['Scans'])}</span>
                    </div>
                    """, unsafe_allow_html=True)
